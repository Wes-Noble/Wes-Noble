...
- Being introduced to tessellations and coverages, our understanding of spatial representation grew and it relationship to the modifiable areal unit problem became clear with point-based analysis
- We developed functions for future use of filtering to the US, plotting spatial features, and creating point-in-polygons
...
- While using a flood event in Palo, Iowa as an example flood, we delved into different normalized differences and how different uses of landsat data layers can be formulated to represent various spatial figures from wavelength frequency detection
- Raster thresholding allowed us to identify flooding regions in a binary format
- After completing multiple normalized different calculations and developing a k-means classification based on patterns in the data we were able to represent flooding probability based on the frequency of calculated flooding events from the normalized differences of the region.
...
- Through the use of rasters and raster functions, we could formulate hillshade data from extracting the elevation of our studied region
- After basing flood extent from past sensor data, we then modeled the flood of Mission Creek and the damage a 10m rise in the water level could do to local buildings
